...
export class SidebarComponent implements OnInit, AfterViewInit {
  menuItems: MenuItem[] = [];
  currentUser: ApiPerson | null = null;
  ...
  ngOnInit(): void {
    ...
    this.navigationService.currentUser$.subscribe(user => {
      this.currentUser = user;
    });

    // Listen to router events to update active menu item
    //TODO: uncomment this listener if nothing works
  /*  this.router.events
  ...
  onMenuItemClick(item: MenuItem, event: MouseEvent): void {
    this.preserveScrollPosition(event);

    // update active state
    this.navigationService.setActiveMenuItem(item.route);

    this.navigationRefreshService.triggerRefresh(item.route);  // ← always trigger

    // trigger data reload for abwesenheit
/*    if (item.route.includes('abwesenheit')) {
