...
export class ErrorHandlingService {
  ...
  handleAppError(err : any){
    let errorMessage = 'An unknown error occurred.';

    console.log('err.status', err.status);
    if (err.status === 0) {
      // Server not reachable (CORS, offline, backend down)
      errorMessage = 'Could not reach the server. Please check your internet connection or try again later.';
    } else if (err.error instanceof ErrorEvent) {
      // Client-side error
      errorMessage = `Client error: ${err.error.message}`;
    } else {
      // Server-side error (e.g., 500, 404, etc.)
      errorMessage = `Server error (${err.status}): ${err.error?.message || 'Unknown error'}`;
    }

    console.error('Error details:', err); 
    console.error('Error details-error:', err.error); 
    this.showError(errorMessage  + ', '  + (err.error != null ? err.error : '')); // Display the message
